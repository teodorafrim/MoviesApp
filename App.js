...
class App extends Component {
  ...
  handleAddToFavorite = (movie) => {
    const isAlreadyInFavorites = this.state.favorites.some((fav) => fav.id === movie.id);

    // Add the movie to the favorites list if it's not already there
        ...
        <div className="movie-grid">
          {filteredMovies.map((movie) => (
            <div className="movie-card" key={movie.id}>
              ...
              {selectedTab !== "favorites" && (
                <div
                className={`favorite-button ${favorites.some((fav) => fav.id === movie.id) ? 'disabled' : ''}`}
                onClick={() => this.handleAddToFavorite(movie)}
                disabled={favorites.some((fav) => fav.id === movie.id)}
              >
                Add to Favorites
              </div>
              )}
